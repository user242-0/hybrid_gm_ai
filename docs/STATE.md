...
## 3. 直近の変更（最新3つだけ）
* 2026-06-20: Session 47-B: 愉快犯FLEE用の基本HUDアクション5件と安全なworld effectsを追加。actor別Actions listboxの非混在と実行をテスト。
* 2026-06-19:Session 46:repo cleanup / LOGBOOK整理 / main整備
* 2026-06-18: Session 45: Action Proposal advisory pipelineに `actor_id` を追加。proposal → shadow → advisory → display item → provider → HUDCallbacks まで actor_id を通し、HUDのAI提案欄は current active_char.name に一致する提案だけを表示できるようにした。引き続きread-only表示で、ActionPipeline / action_registry / Actions listbox には未接続。
...
3. MicroGoalをRC別に独自化する設計を行う。

   * 現状のmode共通micro bankとactor別HUD Actions割当の責務を分離する。
   * 非操作中RCのMicroGoal表示・進行・履歴をどう保持するか決める。

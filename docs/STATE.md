...
1. 既存失敗testを修正し、`pytest -q` の all green を目指す。

   * `tests/test_npc_switch.py`
   * `tests/test_requirements_time_weather.py`

2. all green後、mainブランチへ取り込み、ブランチ整理を行う。

3. その後、RC専用HUD設計へ進む。

   * 現在はAI提案欄のみactor_idで切り替わる。
   * 将来的にはActions listboxも「現在操作中RCが選べるアクション」に寄せる。
   * `switch_character` により、操作RC・AI提案・HUDアクション・物語局面が切り替わる体験を目指す。

...
* 2026-06-17: Session 45: Action Proposal advisory pipelineに `actor_id` を追加。proposal → shadow → advisory → display item → provider → HUDCallbacks まで actor_id を通し、HUDのAI提案欄は current active_char.name に一致する提案だけを表示できるようにした。引き続きread-only表示で、ActionPipeline / action_registry / Actions listbox には未接続。
...
1. HUD AI提案欄の actor 切替を実ゲームで目視確認する

   * `python -m src.action_proposal.demo_seed` で標準shadow logへseed
   * active characterを刑事/愉快犯で切り替える
   * 対応するactor_idの提案だけが表示されることを確認
